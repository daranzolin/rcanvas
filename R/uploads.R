...
#' @param submission_types string - List of supported submission types for the assignment. Unless the assignment is allowing
#' online submissions, the array should only have one element. Options: online_quiz, none, on_paper, discussion_topic,
#' external_tool, online_upload, online_text_entry, online_url, media_recording
#' @param allowed_extensions Allowed extensions if submission_types includes “online_upload”. E.g. "docx", "png".
#' @param turnitin_enabled boolean - Only applies when the Turnitin plugin is enabled for a course and the submission_types
#' array includes “online_upload”. Toggles Turnitin submissions for the assignment. Will be ignored if Turnitin is not
#' available for the course.
#' @param vericite_enabled boolean - Only applies when the VeriCite plugin is enabled for a course and the submission_types
#' array includes “online_upload”. Toggles VeriCite submissions for the assignment. Will be ignored if
#' VeriCite is not available for the course.
#' @param turnitin_settings string - 	Settings to send along to turnitin. See Assignment object definition for format.
#' @param integration_data string - Data used for SIS integrations. Requires admin-level token with the “Manage SIS” permission.
#' JSON string required.
#' @param integration_id string - Unique ID from third party integrations
#' @param peer_reviews boolean - If submission_types does not include external_tool,discussion_topic, online_quiz, or on_paper,
#' determines whether or not peer reviews will be turned on for the assignment.
#' @param automatic_peer_reviews boolean - Whether peer reviews will be assigned automatically by Canvas or if teachers must manually assign peer reviews.
#' Does not apply if peer reviews are not enabled.
...
#' @param grade_group_students_individually boolean - If this is a group assignment, teachers have the options to grade students individually.
#' If false, Canvas will apply the assignment's score to each member of the group. If true, the teacher can manually assign scores to each member of the group.
...
#' @param grading_type string - The strategy used for grading the assignment. The assignment defaults to “points” if this field is omitted.
#' Options: pass_fail, percent, letter_grade, gpa_scale, points
#' @param due_at datetime - The day/time the assignment is due. Must be between the lock dates if there are lock dates.
#' Accepts times in ISO 8601 format, e.g. 2014-10-21T18:48:00Z.
#' @param lock_at datetime - The day/time the assignment is locked after. Must be after the due date if there is a due date.
#' Accepts times in ISO 8601 format, e.g. 2014-10-21T18:48:00Z.
#' @param unlock_at datetime - The day/time the assignment is unlocked. Must be before the due date if there is a due date.
#' Accepts times in ISO 8601 format, e.g. 2014-10-21T18:48:00Z.
...
#' @param muted boolean - Whether this assignment is muted. A muted assignment does not send change notifications and hides grades from students.
#' Defaults to false.
...
#' @param grading_standard_id integer - The grading standard id to set for the course. If no value is provided for this
#' argument the current grading_standard will be un-set from this course. This will update the grading_type for the course to
...
#' @param quiz_lti boolean - Whether this assignment should use the Quizzes 2 LTI tool. Sets the submission type to 'external_tool'
#' and configures the external tool attributes to use the Quizzes 2 LTI tool configured for this course. Has no effect if no Quizzes 2 LTI tool is configured.
#'
#' @return
#' @export
#'
